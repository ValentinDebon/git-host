...
```
groupadd -r git
useradd -r -md /srv/git -g git -s /usr/local/bin/git-host
...
Then, you'll need to create a `Match` rule for the `git` user, you can usually set up a dedicated configuration file in `/etc/ssh/sshd_config.d/git.conf`:
```
Match User git
